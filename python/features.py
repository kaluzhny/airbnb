...
def get_one_hot_columns(x):
    new_columns = [column for column in x.columns_ if column.startswith('bin_')]
...
def add_sessions_features(data_df, sessions_df):
    ...
    # for action in sessions_df['action_detail'].unique():
    #     if isinstance(action, str):
    #         data_df['s_has_action_detail_' + action] = sessions_has_action_detail(data_df, sessions_df, action)
